...
def display_stock_prediction(symbol):
    ...
    if predictions is not None:
        ...
        st.write("\nPrediction Summary:")
        st.write(f"Mean (Average Prediction): ${predictions['Avg_Predicted_Price'].mean():.2f}")
        st.write(f"Min (Average Prediction): ${predictions['Avg_Predicted_Price'].min():.2f}")
        st.write(f"Max (Average Prediction): ${predictions['Avg_Predicted_Price'].max():.2f}")
...
def set_page_theme(is_dark_mode):
    if is_dark_mode:
        st.markdown("""
        <style>
        :root {
...
def main():
    ...
    with st.sidebar:
        is_dark_mode = st.checkbox("Dark Mode", value=True)
    
    # Set the theme based on user preference
    set_page_theme(is_dark_mode)
